...
def to_xgrow(stxg, stream=None):
    ...
    if all(bondhasname):
        xgrowf.write(
            "binding type names={ %s }\n" % " ".join([x["name"] for x in stxg["bonds"]])
        )
    elif any(bondhasname):
        raise ValueError(
            "Some bonds have names, but not all. "
    ...
    if "glues" in stxg:
        for x1, x2, g in [tuple(x) for x in stxg["glues"]]:
            ...
            if type(x1) != int:
                try:
                    x1 = [n for n, z in enumerate(stxg["bonds"]) if z["name"] == x1][
                        0
                    ] + 1
                except IndexError:
                    print(x2)
            ...
            if type(x2) != int:
                try:
                    x2 = [n for n, z in enumerate(stxg["bonds"]) if z["name"] == x2][
                        0
                    ] + 1
                except IndexError:
                    print(x2)
                ...
                for x1, x2 in val:
                    ...
                    if type(x1) != int:
                        try:
                            x1 = [
                                n
                                for n, z in enumerate(stxg["tiles"])
                                if z["name"] == x1
                            ][0] + 1
                        except IndexError:
                            print(x1)
    ...
    xgrowf.write(
        "% Tileset created by stxg.py version {0} on {1}\n".format(
