...
def _run_xgrow( params ):
    ...
    # Move to correct directory on cluster. This needs to be done each
    # time because the nodes need it. In this case, there is a '.xgrowcluster'
    # in the node's user's home folder has the config for each node yaml-encoded.
    ...
    with tempfile.NamedTemporaryFile(mode='r',dir=config['temppath']) as datafile:
        z = subprocess.check_call(
            "./xgrow {0} -nw {1}={2} {3}".format(
                tilefile.name,outtype,datafile.name,paramstring),shell=True)
        data = datafile.read()
...
class XgrowCluster:
    ...
    def do_sims(self, tilestring, paramstring, outtype, num=50):
        ...
        * num (=50 default): number of runs to run.
        """
        outparam = {'final': 'datafile', 'trace': 'tracefile', 'array': 'arrayfile'}[outtype]
...
class SimRunSingleMismatch:
    """
    A class for sets of simulations with single mismatch interactions, varying the strength
    of the interaction for each set.
    """
    def __init__(self, xgcluster, baseset, extraparams,
                 num=50, defvals=[0.0,0.025,0.05,0.075,0.1,0.2,0.4,0.5,0.8]):
        """
        baseset is a tileset file as a string, in a way such that glue info can be appended to it (no params
        at the end).
        extraparams are all the extra parameters
        """
        self.num = num
    ...
    def runfunction(self,g1,g2,strength,n=50):
        ...
        return self.xgc.do_sims(self.baseset+"\ng({},{})={}\n".format(g1,g2,strength),self.extraparams,num=n)

    def add(self, name, g1, g2, vals=None):
        "Add a new simulation set, with interactions between g1 and g2"
        self.res[name] = {}
        if not vals: vals = self.defvals
        for gval in vals:
    ...
    def progress(self):
        """
        Fractional progress of run. Note that this doesn't use the AsyncResult.progress method because
        in normal ipython 0.13 / 0.14 this can sometimes hang for 10 seconds owing to a timeout problem.
        """
        done = 0
        total = 0
    ...
    def map(self, function):
        return dict( (n,array([ [k,function(x)] for k,x in s.items() ])) for n in self.res )
